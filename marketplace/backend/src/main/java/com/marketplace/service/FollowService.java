...
public class FollowService {
    ...
    public void follow(UUID followerId, UUID followedId) {
        ...
        if (followRepository.findByIdFollowerIdAndIdFollowedId(followerId, followedId).isPresent()) {
        ...
        follow.getId().setFollowerId(followerId);
        follow.getId().setFollowedId(followedId);
    ...
    public void unfollow(UUID followerId, UUID followedId) {
        followRepository.deleteByIdFollowerIdAndIdFollowedId(followerId, followedId);
    }

    public List<User> getFollowers(UUID userId) {
        List<Follow> follows = followRepository.findByIdFollowedId(userId);
        List<UUID> followerIds = follows.stream().map(f -> f.getId().getFollowerId()).collect(Collectors.toList());
        return userRepository.findAllById(followerIds);
    }

    public List<User> getFollowing(UUID userId) {
        List<Follow> follows = followRepository.findByIdFollowerId(userId);
        List<UUID> followedIds = follows.stream().map(f -> f.getId().getFollowedId()).collect(Collectors.toList());
